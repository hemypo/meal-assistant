...
- **Last completed:** project docs created (CLAUDE.md, README.md, master plan in `docs/`)
- **Next step:** scaffold `create-next-app` (TypeScript, Tailwind, App Router) → push to GitHub → connect Vercel → create Neon DB → init Prisma with the `User` model → first migration
- **Known issues:** none
- **Waiting on founder:** Gemini API key and Neon account creation (founder performs all credential entry himself; never ask him to paste secrets into chat)
