...
- **Waiting on founder:** Nothing blocking. Open items he may want to act on: the dev-account password chosen by the agent (`provizia-dev-2026`) should be replaced — cleanest is deleting that user so he registers with his own; the inventory holds ~27 agent test products that can be cleared; harness installs are now **done** (ECC rules, karpathy-guidelines, ui-ux-pro-max in .claude/), but ui-ux-pro-max needs **Python 3.x** installed to run its search engine. Founder performs all credential entry himself; never ask him to paste secrets into chat.
