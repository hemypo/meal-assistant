...
- **Current phase:** Phase 0 — Foundations ✅ **COMPLETE** (both accept criteria met). Next up: Phase 1 — Auth + Inventory core (master plan §10).
- **Last completed:** Phase 0 done end-to-end. Next.js 16 + TypeScript + Tailwind + App Router scaffold on `main`; Prisma 7 with only the `User` model, `prisma.config.ts`, and the `@prisma/adapter-neon` singleton at `src/lib/db.ts`; initial migration `20260802083048_init` applied to the Neon `production` branch and **committed**; GitHub `hemypo/meal-assistant` (private) synced; Vercel production deploy green and live at **https://meal-assistant-nine.vercel.app**. Adapter path verified at runtime via a temporary `/api/health` route returning `{"ok":true,"users":0}` (route since removed).
- **Live infrastructure:** Neon org `org-rough-band-22135740` (free) → project `meal-assistant` = `sparkling-wildflower-51955601`, PG 18, `aws-us-east-2`, branch `production` = `br-solitary-brook-axo7lr7j`. Vercel project `prj_9KQrtAtQEmWLVIpx1TAMHcIYsCDI`, deploys via GitHub integration. CLIs `vercel`, `neon`/`neonctl`, and `gh` are all installed and authenticated as `hemypo`.
- **Next step:** Phase 1 — Auth.js v5 Credentials + argon2, registration locked to `ALLOWED_EMAIL`; Product CRUD with the two-status model; category grouping + alphabetical sort; mobile-first bottom-nav shell per `design-system/MASTER.md`. Accept criteria in master plan §10 Phase 1. Start with `/ecc:plan` per §6.2, and read `design-system/MASTER.md` before any UI work.
- **Known issues:** The Prisma 7 + Turbopack module-resolution concern logged earlier is **disproven for our setup** — the real adapter path was exercised successfully against Neon. Remaining watch-items: (a) `npm audit` reports 3 high-severity advisories inside Next.js 16.2.12's bundled `postcss`/`sharp`; npm's suggested "fix" downgrades Next to 9.3.3, so it is **not** applied — revisit when a Next patch ships; (b) if the Neon password is rotated, `DATABASE_URL`/`DIRECT_URL` in Vercel need manual updating (see §7); (c) Vercel `PG*` vars exist in Preview too, but `DATABASE_URL`/`DIRECT_URL`/`AUTH_SECRET`/`ALLOWED_EMAIL` were set for **Production only** — add them to Preview before relying on branch preview deployments.
- **Waiting on founder:** Gemini API key from Google AI Studio (`GEMINI_API_KEY`) — not needed until Phase 2, but grab it when convenient. Founder performs all credential entry himself; never ask him to paste secrets into chat.
