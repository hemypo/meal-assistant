...
- **Current phase:** Phase 4 — Receipt scanner (photo + text) ✅ **COMPLETE** (both accept criteria verified). Next up: Phase 4b — QR / fiscal-data source, or Phase 5 — Finance + weight (master plan §10). 4b needs `RECEIPT_API_KEY`, which does not exist yet, so **Phase 5 is the unblocked path**.
