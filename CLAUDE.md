...
> **⛔ PHASE GATE (founder instruction, 2026-08-03).** Each phase needs explicit consent before it starts; finish, report, then stop and ask. **Consent currently given for Phase 6** ("you can continue the phases", 2026-08-03) — Phase 8 (finance assistant) is NOT covered and must be asked for separately.


- **Current phase:** Phase 7 — Settings + personalised nutrition targets ✅ **COMPLETE** (delivered out of order at the founder’s request). Phase 6 (hardening) is **authorised and next**.
- **Last completed:** Settings panel at `/settings`. `User` gained sex / birthYear / heightCm / activityLevel / goal + Б/Ж/У targets (migration `20260802191133_add_nutrition_profile`); `src/lib/nutrition.ts` computes BMR/TDEE/target by Mifflin-St Jeor with a health floor; `explain_nutrition` task added to the registry; `GET/PATCH /api/settings` and `POST /api/settings/plan`; nav gained a utility Settings entry without breaking the five-destination rule. `MenuView`’s hardcoded 2000 is gone and `generate_recipe` now receives a per-meal calorie target. README rows 19–20 added. **109 tests green**, lint clean, build green.
- **Verification evidence (live, real DB + real Gemini):** with weight 78,1 / male / 1994 / 180 cm / moderate / lose, the API returned BMR **1751**, TDEE **2714**, recommended **2307** — each matching an independent hand-calculation. Before the profile existed the estimate was `null` rather than an invented number, and the stored target stayed 2000 until explicitly applied. The AI plan returned Б173/Ж77/У231 = 2309 kcal against the 2307 target (2 kcal drift, inside tolerance, so no fallback). After applying, a generated recipe came back at **692 kcal against an expected 692** — the target now genuinely reaches recipe generation.
- **Next step:** Phase 6 — hardening: PWA manifest + icons (installable on the phone), Sentry, empty/loading/error states everywhere, the 7-day cleanup cron for orphan AI recipes, backup-restore test, and the §7 security checklist walked top to bottom. Master plan §10 Phase 6. **Authorised.**
- **Known issues:** (a) `npm audit` reports 3 high-severity advisories inside Next.js 16.2.12’s bundled `postcss`/`sharp`; the suggested "fix" downgrades Next to 9.3.3, so **not** applied. (b) Neon password rotation requires manually updating `DATABASE_URL`/`DIRECT_URL` in Vercel. (c) Vercel app env vars are **Production only** — add to Preview before relying on branch previews. (d) Sidebar and bottom bar share one `aria-label`; only one renders at a time. (e) The rate limiter is in-memory — resets on cold start, does not span serverless instances; needs Upstash Redis before real traffic. (f) Vercel env vars are **Sensitive**, so `vercel env pull` returns the literal `[SENSITIVE]` — never pull into `.env.local`, it shadows `.env`. (g) ui-ux-pro-max’s `search.py` needs **Python 3.x**, absent here. (h) **Neon free-tier cold start**: the first DB call after idle fails with P1001; builds are protected by `scripts/migrate-deploy.mjs` but *runtime* requests are not — the first page load after a quiet spell can still error. Worth a retry in `db.ts` during Phase 6. (i) Merge-on-confirm matches by normalised name, so two products with an identical name would only merge one. (j) Recharts 3 cannot grow a hovered donut segment; hover dims the others instead. (k) The per-meal share for recipe targeting is a flat 30% — breakfast and dinner are not really equal, but the recipe is not tied to a slot at generation time.
