...
- **Current phase:** Phase 3 — Recipes + meal calendar ✅ **COMPLETE** (§9.2 loop verified end to end). Next up: Phase 4 — Receipt scanner, photo + text (master plan §10).
- **Last completed:** Phase 3 shipped. `Recipe` / `RecipeIngredient` / `MealPlanEntry` + `RecipeSource`/`MealType` enums + `User.kcalTarget` (migration `20260802123120_add_recipes_and_meal_plan`); `generate_recipe` (main tier) and `estimate_kbju` (cheap tier) added to the task registry; `POST /api/ai/recipe` loads the pantry server-side; recipes/meal-plan CRUD under `src/server/recipes.ts` + `src/server/mealplan.ts`; full Рацион UI per MASTER.md — week grid flowing 4+3, DayCard with 2×2 slots and kcal ring, 440px RecipePanel slide-over with Esc, «Шеф-повар Gemini», «Свой рецепт», «Мои рецепты». README rows 8–12 → ✅. **68 tests green**, lint clean, build green, bundle scan clean.
- **Accept criteria evidence (§9.2 loop, run against the real DB):** generate from stock → 201 in 4.6s («Творожный десерт с яблоком и авокадо», 780 ккал, 6 steps, 1 missing, `isSaved:false`, `source:AI`); save → `isSaved:true` and appears in «Мои рецепты»; schedule → 201 on 2026-08-02 BREAKFAST; week query returns 1 entry totalling 780 ккал; missing → закупки added 1; **re-running it added 0 and reported `alreadyListed:1`** (no duplicates). Manual entry: `estimate_kbju` returned 485 ккал with a 1-kcal arithmetic error, saved as `MANUAL`. Verified visually at 1440px (sidebar + 4+3 grid + panel) and 375px (single column + bottom bar).
...
- **Next step:** Phase 4 — Receipt scanner (photo + text). Add `Receipt`/`ReceiptItem` + `ReceiptStatus`/`ReceiptSource` per master plan §4; Gemini **Vision** parsing plus the same schema for pasted text; draft review/edit screen; and the **atomic confirm** (§9.1) — that one genuinely needs `$transaction`, since items merge into `Product(IN_STOCK)` *and* an `Expense` is created *and* the receipt flips to `CONFIRMED`, all or nothing. Accept: a real photographed receipt updates inventory and creates an expense in one confirm tap; a discarded draft leaves zero traces. Needs Vercel Blob for image storage (max 5 MB, jpeg/png/webp/heic per §7) and client-side compression before upload. Read MASTER.md §5 ReceiptDraftRow / ConfirmBar / ReceiptHistoryCard / «AI draft note» / «Empty draft state» first — they are specced in unusual detail.
- **Known issues:** (a) `npm audit` reports 3 high-severity advisories inside Next.js 16.2.12's bundled `postcss`/`sharp`; npm's "fix" downgrades Next to 9.3.3, so **not** applied. (b) Neon password rotation requires manually updating `DATABASE_URL`/`DIRECT_URL` in Vercel (§7). (c) Vercel app env vars are **Production only** — add to Preview before relying on branch previews. (d) Sidebar and bottom bar share one `aria-label`; only one renders at a time. (e) `/receipts`, `/finance`, `/weight` are honest phase placeholders. (f) The rate limiter is in-memory — resets on cold start, does not span serverless instances; needs Upstash Redis before real traffic. (g) Vercel env vars are marked **Sensitive**, so `vercel env pull` returns the literal `[SENSITIVE]` — never pull into `.env.local`, it shadows `.env` and breaks local dev. (h) ui-ux-pro-max's `search.py` needs **Python 3.x**, absent here (`python` is the Microsoft Store stub). (i) `User.kcalTarget` has no settings UI yet — the 2000 default is used until Phase 5. (j) The browser preview pane sometimes navigates back to `/inventory` between tool calls, so multi-step UI checks are best done as one atomic script.
