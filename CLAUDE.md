...
- **Current phase:** Phase 5 — Finance analytics + weight ✅ **COMPLETE** (accept criterion verified). **All feature phases are done.** Only Phase 6 (hardening) remains — and it is GATED: do not start it without the founder saying so.
- **Last completed:** Phase 5 shipped. `WeightEntry` (unique on `userId,date`) + `User.weightGoalKg` default 76,0 — migration `20260802172737_add_weight_and_goal`; `src/server/analytics.ts` (expense CRUD, totals, category split, daily trend) and `src/server/weight.ts` (one-per-day upsert, goal distance, average daily ккал pulled from the meal calendar); routes `GET/POST /api/expenses`, `DELETE /api/expenses/[id]`, `GET /api/analytics/summary`, `GET/POST /api/weights`; Recharts donut + trend + weight chart with a dashed goal line, StatCards, PeriodNav by month, manual-expense modal, and the WeightEntryForm that MASTER.md designates as the template for all single-row add forms. README rows 15–16 → ✅. **99 tests green**, lint clean, build green.
- **Accept criterion evidence ("charts match a hand-checked sum of the rows"):** seeded four known expenses alongside the real receipt-derived one, then recomputed independently from the raw rows in the browser: **total 2699,90 = API total**, every category amount matched, every daily trend bucket matched, categories sorted by amount desc and trend ascending by date. Weight: three entries stayed three after re-posting the same date, and the value was **replaced** 78,4 → 78,1 (one-per-day upsert works); goal 76,0, До цели 2,1 кг, progress 50% — correct for a start of 80,2 against a 4,2 kg gap; a 12 kg entry was rejected 400 «Вес должен быть от 30 до 250 кг»; `avgKcal` 780 confirmed the weight view really reads the Phase 3 meal calendar. Seeded expenses were deleted afterwards, leaving only the genuine receipt expense.
- **Next step:** Phase 6 — hardening (PWA manifest + icons, Sentry, empty/loading/error states, the 7-day cleanup cron for orphan AI recipes, backup-restore test, security checklist top to bottom). **⛔ Do not begin without explicit founder consent** (see the gate at the top of this section). Master plan §10 Phase 6 has the full list.
- **Known issues:** (a) `npm audit` reports 3 high-severity advisories inside Next.js 16.2.12’s bundled `postcss`/`sharp`; npm’s "fix" downgrades Next to 9.3.3, so **not** applied. (b) Neon password rotation requires manually updating `DATABASE_URL`/`DIRECT_URL` in Vercel (§7). (c) Vercel app env vars are **Production only** — add to Preview before relying on branch previews. (d) Sidebar and bottom bar share one `aria-label`; only one renders at a time. (e) **All five modules are now real** — no placeholder pages remain. (f) The rate limiter is in-memory — resets on cold start, does not span serverless instances; needs Upstash Redis before real traffic. (g) Vercel env vars are marked **Sensitive**, so `vercel env pull` returns the literal `[SENSITIVE]` — never pull into `.env.local`, it shadows `.env` and breaks local dev. (h) ui-ux-pro-max’s `search.py` needs **Python 3.x**, absent here. (i) **Neither `User.kcalTarget` nor `User.weightGoalKg` has a settings UI** — the 2000 / 76,0 defaults are used, so the founder cannot change his own goals yet. This is the most visible gap left; MASTER.md §9.2 flagged it as needing "a small settings surface". (j) The browser preview pane sometimes navigates back to `/inventory` between tool calls. (k) **Neon free-tier cold start**: the first DB call after idle fails with P1001. Builds are protected by `scripts/migrate-deploy.mjs`, but *runtime* requests are not — the first page load after a quiet spell can still error. (l) Merge-on-confirm matches by normalised name, so two products with an identical name would only merge one.
