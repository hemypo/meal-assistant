...
- 2026-08-05 — **Refactor Stage 1: the receipt confirm had a real double-spend race.** The DRAFT guard sat *outside* the transaction, so two concurrent confirms both passed it and both applied — reproduced by a test that models Postgres row semantics and failed with **«expected 2 to be 1»** (two expenses). `Expense.receiptId @unique` had been containing the damage, but via a constraint violation surfacing as a 500 rather than by design. Fixed by claiming the receipt as the **first statement inside** the transaction (`updateMany` conditional on `status: "DRAFT"`); the loser matches zero rows, throws `AlreadyConfirmedError`, rolls back before any money is written, and returns a clean `NOT_DRAFT`. Tests written **before** the fix and shown failing first.
- 2026-08-05 — Same stage: the per-item `tx.product.update` loop inside the confirm transaction was a genuine N+1 — a test proved **30 items → 30 statements**. Now grouped by quantity into one `updateMany` per distinct value (30 items / 3 quantities → 3 statements). Chose ORM grouping over raw `UPDATE ... FROM (VALUES ...)`: it keeps type safety, and real receipts cluster heavily around quantity 1–2. Worst case (all-distinct quantities) is still O(n) — acceptable, and documented rather than hidden.
- 2026-08-05 — Updating the existing receipt tests for this change was deliberate, not test-weakening: every **behavioural** assertion (expense amount, merge-vs-create counts, single transaction, rollback, no confirm outside the transaction) was preserved; only assertions naming specific Prisma methods (`product.update` → `product.updateMany`, `receipt.update` → the conditional claim) moved, because those pinned the implementation rather than the contract.
- 2026-08-05 — **Known environment limitation, not a code defect:** this sandbox cannot resolve the **pooled** Neon hostname (`EAI_AGAIN`) while the direct host resolves fine, and `fonts.googleapis.com` is likewise unresolvable. Consequence: `npm run build` cannot complete locally (Manrope fetch) and the app cannot reach the DB locally, so Stage 1 was verified by 144 unit tests + typecheck + lint, with live verification done against production after deploy. Also worth noting: the build has a hard build-time dependency on Google Fonts — self-hosting Manrope via `next/font/local` would remove that single point of failure.
