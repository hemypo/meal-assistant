...
- **Current phase:** Phase 2 — AI gateway + smart inputs ✅ **COMPLETE** (both accept criteria verified). Next up: Phase 3 — Recipes + meal calendar (master plan §10).
- **Last completed:** Phase 2 shipped. `src/lib/ai/{client,schemas,tasks}.ts` behind `server-only`; single gateway `POST /api/ai/assist` with a 4-task registry (`categorize`, `parse_bulk_list`, `suggest_unit`, `estimate_price`); `CategoryMapping` cache (migration `20260802115417_add_category_mapping`); weighted in-memory rate limiter (20/min/user, bulk parse costs 3); `BulkAddModal` on a native `<dialog>`; AI suggestion chips wired into the product form for category (passive, 500ms debounce), unit and price (explicit ✨). README rows 4–7 → ✅. **38 tests green**, lint clean, build green. Phase 1 before it: auth + inventory core, rows 1–3.
- **Accept criteria evidence:** (1) *One visible AI step for 20 lines* — pasted a 20-line messy Russian list; a single `POST /api/products/bulk` returned all 20 drafts, **20/20 correctly parsed and categorised** (incl. «хлеб» with no quantity → 1 шт, «8 рулонов» → 8 шт mapped onto the fixed unit list, «пельмени»/«брокколи» → Заморозка); confirm saved 20/20 in ~1.3s. (2) *Key absent from the browser bundle* — scanned 19 built client files: zero occurrences of `GEMINI_API_KEY`, `AUTH_SECRET`, the DB password, `GoogleGenAI`, `PrismaClient`, or even the `generativelanguage.googleapis.com` host. Cache corroborated live: a repeat parse of known names dropped 5.0s → 2.8s (one fewer AI round-trip).
...
- **Next step:** Phase 3 — AI recipe generation from `IN_STOCK` products with free-text wishes; manual recipe entry with КБЖУ assist; «Мои рецепты» (`isSaved`); missing ingredients → shopping list with estimated prices; week calendar (real dates, Mon–Sun × Завтрак/Обед/Ужин/Перекус) with daily КБЖУ totals. Accept: the full §9.2 loop works end to end. Add `Recipe`, `RecipeIngredient`, `MealPlanEntry` per master plan §4. **Consider `reasoning: true` for the recipe task** — unlike Phase 2's extraction work, generation may genuinely benefit from thinking; measure both ways before deciding. Read `design-system/MASTER.md` §5 RecipePanel/DayCard/KbjuBadges and §4 (4-column wrapping day grid) first.
- **Known issues:** (a) `npm audit` reports 3 high-severity advisories inside Next.js 16.2.12's bundled `postcss`/`sharp`; npm's suggested "fix" downgrades Next to 9.3.3, so it is **not** applied — revisit when a Next patch ships. (b) If the Neon password is rotated, `DATABASE_URL`/`DIRECT_URL` in Vercel need manual updating (§7). (c) Vercel app env vars are set for **Production only** — add them to Preview before relying on branch preview deployments. (d) The sidebar and bottom bar share `aria-label="Основная навигация"`; only one is ever displayed, so assistive tech sees one. (e) `/menu`, `/receipts`, `/finance`, `/weight` are honest "появится в фазе N" placeholders, not stubs pretending to work. (f) The rate limiter is in-memory: it resets on cold start and does not span serverless instances — fine for one user, needs Upstash Redis before real traffic. (g) Vercel env vars are marked **Sensitive**, so `vercel env pull` returns the literal string `[SENSITIVE]` instead of real values — never pull into `.env.local`, it shadows `.env` and breaks local dev.
- **Waiting on founder:** Nothing blocking. Open items he may want to act on: the dev-account password chosen by the agent (`provizia-dev-2026`) should be replaced — cleanest is deleting that user so he registers with his own; the inventory holds ~27 agent test products that can be cleared; and the approved harness-tool installs (ECC rules, karpathy-skills, ui-ux-pro-max) were **not performed** — the clone step was declined mid-run, so agree a fetch method before retrying. Founder performs all credential entry himself; never ask him to paste secrets into chat.
