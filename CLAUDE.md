...
> **⛔ PHASE GATE (founder instruction, 2026-08-03).** Each phase needs explicit consent before it starts; finish, report, then stop and ask. **No consent currently outstanding** — Phase 6 is done. Phase 8 (finance assistant) and anything else must be asked for separately.


- **Current phase:** Phase 6 — Hardening ✅ **COMPLETE**. All feature phases (1–7) are now done; the only open build work is Phase 8 (finance assistant), which is **planned but NOT authorised**.
- **Last completed:** Phase 6. PWA is installable — `src/app/manifest.ts` + generated 192/512/maskable/apple-touch icons, verified live (all icons serve real PNGs, `installable: true`). Route boundaries added: `(app)/error.tsx`, `(app)/loading.tsx`, `not-found.tsx`, `global-error.tsx`. Every list view now distinguishes a failed load from an empty one. Runtime Neon cold-start retry in `db.ts`. Orphan-recipe cleanup cron. Security checklist walked with evidence. `src/lib/observability.ts` added as the single error-reporting seam. **122 tests green**, lint clean, build green, bundle scan clean across all five secrets.
- **Security walk evidence (master plan §7):** `.env` git-ignored and `.env.example` holds names only; registration locked to `ALLOWED_EMAIL` (fails closed if unset); argon2 hash/verify, never plaintext; **19/20 API routes require a session** — the exception is `/api/register`, which must work logged-out and still Zod-validates and enforces the email lock; the three routes without Zod take **no body** (parameterless POSTs whose only input is a path id used in a `userId`-scoped query); rate limiting on all five AI paths; **no `$queryRawUnsafe`** anywhere in our code; upload capped at 5 MB with a jpeg/png/webp/heic whitelist; every `prisma` call in `src/server/` is either `userId`-scoped, guarded by a prior ownership check, or the documented cron exception; no prompt sends email, password or session data to Gemini. Bundle scan of 31 client files: zero hits for `GEMINI_API_KEY`, `AUTH_SECRET`, `BLOB_READ_WRITE_TOKEN`, `CRON_SECRET`, the DB password, `PrismaClient`, `GoogleGenAI`, or the Gemini hostname.
- **Next step:** nothing is authorised. Phase 8 (finance assistant) is specced in master plan §10 and awaits explicit consent. Two smaller items are also open and unauthorised: wiring Sentry once the founder has an account (README row 18), and the Neon backup-restore drill, which is a console exercise the founder should run himself.
- **Known issues:** (a) `npm audit` reports 3 high-severity advisories inside Next.js 16.2.12’s bundled `postcss`/`sharp`; the suggested fix downgrades Next to 9.3.3, so **not** applied. (b) Neon password rotation requires manually updating `DATABASE_URL`/`DIRECT_URL` in Vercel. (c) Vercel app env vars are **Production only** — add to Preview before relying on branch previews. (d) Sidebar and bottom bar share one `aria-label`; only one renders at a time. (e) The rate limiter is in-memory — resets on cold start, does not span serverless instances; needs Upstash Redis before real traffic. (f) `vercel env pull` returns the literal `[SENSITIVE]` for this project’s vars — never pull into `.env.local`, it shadows `.env`. (g) ui-ux-pro-max’s `search.py` needs **Python 3.x**, absent here. (h) Merge-on-confirm matches by normalised name, so two products with an identical name would only merge one. (i) Recharts 3 cannot grow a hovered donut segment; hover dims the others instead. (j) The per-meal share for recipe targeting is a flat 30%. (k) Sentry is not installed — see §7 and README row 18.
